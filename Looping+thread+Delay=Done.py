...
def search_keywords(keywords, search_retries, results, delay):
    unique_domains_set = set()
    for keyword in keywords:
        print(f"===== Keyword: {keyword} =====")
        for _ in range(search_retries):
            domains, result_count = get_links_from_search(keyword)
            unique_domains_set.update(domains)
            print(f"Jumlah hasil untuk '{keyword}': {result_count}")
            time.sleep(delay)  # Menambahkan delay
    results.append(list(unique_domains_set))  # Menambahkan hasil sebagai list

if __name__ == "__main__":
    filename = input("Masukkan nama file yang berisi kata kunci: ")
    search_retries = int(input("Jumlah pencarian ulang: "))
    num_threads = int(input("Jumlah thread: "))  # Meminta pengguna untuk mengatur jumlah thread
    delay = float(input("Delay (detik) antara setiap pencarian: "))  # Meminta pengguna untuk mengatur delay
    ...
    try:
        ...
        # Membagi pekerjaan pencarian ke dalam thread
        for i in range(0, len(keywords), num_threads):
            batch_keywords = keywords[i:i + num_threads]
            thread = threading.Thread(target=search_keywords, args=(batch_keywords, search_retries, results, delay))
        ...
        for result_set in results:
            unique_domains_set.update(result_set)

        # Menulis domain-domain unik ke file output tanpa menghapus isinya
